...
import { getWorkouts } from '../data/storage'
...
function WorkoutCard({ workout }) {
  ...
  function handleShare() {
    const text = `${workout.routineLabel} Workout — ${formatDate(workout.date)}`
    if (navigator.share) {
      navigator.share({ title: 'Snotra Workout', text })
    } else {
  ...
  return (
    <div className="bg-zinc-900 rounded-2xl px-4 py-4 mb-4">
      ...
      <div className="flex items-center justify-between mt-5 px-4">
        ...
        <button onClick={handleShare} className="text-zinc-400 active:text-white transition-colors active:scale-90">
